...
class KNearestNeighbor:
    ...
    def compute_distances_no_loops(self, X):
        ...
        Xtrain_sq = np.sum(np.square(self.X_train), axis=1, keepdims=True)

        # Calculate dists as sqrt((X_train - X)**2) = sqrt(X_train**2 - 2*X_train*X + X**2)
    ...
    def predict_labels(self, dists, k=1):
        ...
        for i in range(num_test):
            ...
            knn_i = np.argsort(dists[i])[:k]
            closest_y = self.y_train[knn_i]
